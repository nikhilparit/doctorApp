...
const ManageLab = ({ route }) => {
    ...
    const readData = () => {
        ...
        setLoading(true);
        try {
            Axios.post(`${Api}Setting/GetClinicLabSupplier`, formData).then(resp => {
                console.log('Get Clinic Lab Supplier', resp.data);
                setManageLab(resp.data);
                setLoading(false);
            });
        } catch (error) {
            //console.log(error);
        ...
        <>
                    ...
                    <View>
                        ...
                        <View>
                            {ManageLab.length == 0 ?
                                ...
                                ManageLab.map(item => (
                                    <TouchableOpacity onPress={() => openModal(item.labSupplierID)} key={item.labSupplierID}>
                                        <View style={Styles.containerForSubHeader} key={item.labSupplierID}>
            ...
            <Fab
                ...
                renderInPortal={false} size={'md'} icon={<Icon
                    ...
                    color={"white"}
                />}
                onPress={() => navigation.navigate('AddLabWorkBasicDetails', {
                    SupplierID: ''
                })}
            />
            ...
            <Modal isOpen={ShowModal} onClose={() => setShowModal(false)} size={"lg"}>
                <Modal.Content>
                    ...
                    <Modal.Body padding={5}>
                        <TouchableOpacity style={{ margin: 9 }} onPress={() => navigation.navigate('AddLabWorkBasicDetails', {
