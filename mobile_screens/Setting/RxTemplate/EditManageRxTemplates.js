...
const EditManageRxTemplates = ({ route }) => {
    const myClinicID = route.params.clinicID;
    const myprescriptionTemplateMasterID = route.params.prescriptionTemplateMasterID;
    //console.log(myClinicID,myprescriptionTemplateMasterID);
    ...
    const readData = async () => {
        ...
        const formData2 = {
            clinicId: myClinicID, 
            prescriptionTemplateMasterID:myprescriptionTemplateMasterID
        }
        try {
            Axios.post(`${Api}Setting/GetAllTemplateDetailsByMasterID`, formData2).then(resp => {
                console.log(resp.data);
                setselectedDrug(resp.data);
                const {
                    templateName
                } = resp.data[0];
                    ...
                    {selectedDrug.map(item =>
                        <TouchableOpacity onPress={() => openModal1(item.drugId)}>
                            <View style={{ flexDirection: 'row', marginTop: 10, marginLeft: 5, backgroundColor: "#fff", justifyContent: 'space-between' }}>
                                ...
                                <Text style={{ fontFamily: 'Poppins-Regular', fontSize: 12 }}>{item.genericName || item.medicineName}</Text>

                                <Text style={{ fontFamily: 'Poppins-Regular', fontSize: 12 }}>{item.dosageID || item.dosage}</Text>
...
export default EditManageRxTemplates;
