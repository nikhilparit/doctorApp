...
const SMSReport = () => {
    ...
    //data-binding
    const [SMS, setSMSReport] = useState([]);
    const [clinicID, setClinicID] = useState('');
    const [patientID, setPatientID] = useState('');
    //Year wise start date, end date calculations
    const currentYear = moment().format('YYYY');
    const myStartDate = moment(`${currentYear}-01-01`);
    const myEndDate = moment(`${currentYear}-12-31`);
    //search bar 
    ...
    const readData = async () => {
        ...
        const formData = {
            ...
            smsSituationID: "00000000-0000-0000-0000-000000000000",
            smsType: "",
            emailType: "",
            communicationGroupMasterGuid: "00000000-0000-0000-0000-000000000000"
        }
       // console.log('Data To Pass SMS Api', formData);
        try {
            Axios.post(`${Api}Report/GetSMSTransactionReport`, formData, { headers }).then(resp => {
               // console.log('SMS Report', resp.data);
                setSMSReport(resp.data);
                setLoader(false);
            });
        } catch (error) {
           // console.log(error);
    ...
    const handleViewForDate = async (ViewForDate) => {
        ...
        switch (ViewForDate) {
            case "Day View":
              ...
              //  console.log(ViewForDate);
                ...
                const formData = {
                    ...
                    endDate: moment(todayDate).add(1, 'day').format('YYYY-MM-DD'),
                    sortOrder: "DESC",
                    sortColumn: "",
                    pageIndex: 0,
                    pageSize: 0,
            ...
            case "Week View":
               ...
               // console.log('Data To Pass SMS Api', formData1);
                try {
                    Axios.post(`${Api}Report/GetSMSTransactionReport`, formData1, { headers }).then(resp => {
                       // console.log('SMS Report', resp.data);
                        setSMSReport(resp.data);
                        setLoader(false);
                    });
            ...
            case "Month View":
               ...
               // console.log('Data To Pass SMS Api', formData2);
                try {
                    Axios.post(`${Api}Report/GetSMSTransactionReport`, formData2, { headers }).then(resp => {
                       // console.log('SMS Report', resp.data);
                        setSMSReport(resp.data);
                        setLoader(false);
                    });
        ...
        if (viewForDate == "Day View" || viewForDate == '') {
           ...
           // console.log(nextDate);
            const formData = {
                clinicID: clinicID,
                startDate: nextDate,
                endDate: moment(nextDate).add(1, 'days').format('YYYY-MM-DD').toString(),
                sortOrder: "DESC",
                sortColumn: "",
                pageIndex: 0,
                pageSize: 0,
        ...
        if (viewForDate == "Day View" || viewForDate == '') {
           ...
           // console.log(backDate);
            const formData = {
                clinicID: clinicID,
                startDate: backDate,
                endDate: moment(backDate).add(1, 'days').format('YYYY-MM-DD').toString(),
                recordCount: 0,
                pageIndex: 0,
                pageSize: 0,
                sortOrder: "DOB",
        ...
        if (selectedDateType === 'start') {
          ...
          //  console.log(currentDate);
           ...
           // console.log("Final Date", finalDate2);
            ...
            ///console.log('Data To Pass SMS Api', formData);
            try {
                Axios.post(`${Api}Report/GetSMSTransactionReport`, formData).then(resp => {
                   // console.log('SMS Report', resp.data);
                    setSMSReport(resp.data);
                    setLoader(false);
                });
            ...
            {loader ? <Loader /> :
                <ScrollView>
                    <SafeAreaView style={Styles.container}>
                        ...
                        <View>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', backgroundColor: '#e0e0e0', height: 50, marginLeft: 5, marginRight: 10 }}>
                                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'flex-start' }}>
                                    <Text style={{ color: '#000', fontFamily: 'Poppins-Regular', fontSize: regularFontSize, marginLeft: 12 }}>Name</Text>
                                </View>
                                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                                    <Text style={{ color: '#000', fontFamily: 'Poppins-Regular', fontSize: regularFontSize }}> Date</Text>
                                </View>
                                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'flex-end', marginRight: 15 }}>
                                    <Text style={{ color: '#000', fontFamily: 'Poppins-Regular', fontSize: regularFontSize }}>Status</Text>
                                </View>
                            </View>
                            <View>
                                {SMS.length == 0 ?
                                    ...
                                    SMS.map(item => (
                                        ...
                                        //     <View style={Styles.containerForSubHeader} key={item.patientID}>
                                        //         <View>
                                        //             <Text style={Styles.headerSubText1}>
                                        //                 {item.patientName}
                                        //             </Text>
                                        //         </View>
                                               
                                        //         <Text style={Styles.headerSubText}>
                                        //             {moment(item.sentOn).format('LL')}
                                        //         </Text>
                                                
                                        //         <Text style={Styles.headerSubText}>
                                        //             {item.sentStatusMessage}
                                        //         </Text>
                                        //     </View>
                                        // </TouchableOpacity>
                                        <TouchableOpacity onPress={() => openModal(item.patientID)}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', backgroundColor: '#fff', marginTop: 10, height: 50, borderRadius: 5, marginLeft: 5, marginRight: 10, shadowColor: '#000', shadowOffset: { width: 1, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, }} key={item.patientID}>

                                        <View style={{ flex: 1, justifyContent: 'center' }}>
                                            <Text style={{ fontFamily: 'Poppins-Regular', fontSize: regularFontSize, marginLeft: 10 }}> {item.patientName}</Text>

                                                </View>
                                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                                            <Text style={{ fontFamily: 'Poppins-Regular', fontSize: regularFontSize }}>{moment(item.sentOn).format('LL')}</Text>
                                        </View>
                                       
                                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'flex-end', marginRight: 15 }}>
                                            <Text style={{ fontFamily: 'Poppins-Regular', fontSize: regularFontSize  }}> {item.sentStatusMessage}</Text>
                                        </View>
                                        <TouchableOpacity onPress={() => openModal(item.patientID)}>
                    ...
                    <Modal.Body>
                        <VStack space={3}>
                            ...
                            <HStack alignItems="center" justifyContent="space-between">
                                <Text fontWeight="medium">Send Bithday SMS</Text>
                            </HStack>
                            <Divider/>
                            ...
                            <TouchableOpacity onPress={() => viewPatientDetails()}>
                                <HStack alignItems="center" justifyContent="space-between">
                                    <Text fontWeight="medium">View Patient details</Text>
                                </HStack>
                            </TouchableOpacity>
